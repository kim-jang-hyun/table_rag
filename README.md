...
PDF 문서 속 **테이블 구조**를 정확히 이해하는 RAG 시스템입니다.
기존 RAG는 PDF를 단순 텍스트로 파싱하면서 표의 행/열 관계를 파괴합니다.
이 프로젝트는 테이블을 별도 청크로 구조화하고, LangGraph 기반으로 항상 벡터 DB를 먼저 검색한 뒤 문서가 부족할 때만 웹 검색으로 폴백합니다.
...
일반적인 RAG 시스템은 PDF를 좌→우, 위→아래로 읽어 단순 텍스트로 변환합니다.
표가 있는 PDF에서는 이 과정에서 **행/열 관계가 파괴**되어 수치 정보를 잘못 추출합니다.
...
| # | 핵심 아이디어 | 문제 | 해결 방법 |
|---|--------------|------|-----------|
| 1 | **테이블 구조화 저장** | 기존 RAG는 표를 단순 텍스트로 변환해 행/열 관계가 파괴됨 | PDF의 선·배경색·텍스트 정렬 패턴으로 테이블 영역을 감지해 일반 텍스트와 분리 → `열이름=값` 형식으로 구조를 보존하여 별도 청크로 저장 |
| 2 | **복잡한 셀 병합 테이블 인식** | rowspan·colspan이 섞인 표는 단순 텍스트 파싱 시 셀 값이 잘못된 열에 매핑되거나 누락됨 | 병합 셀을 확장해 모든 행의 열 수를 균일하게 맞춘 뒤, 첫 행을 헤더로 고정하고 `열이름=값` 쌍으로 직렬화, 빈 셀(병합 잔여 셀)은 건너뛰어 노이즈 제거 |
...
| 10 | `8744eb6` | Streamlit Cloud 배포 | 멀티 PDF 업로드, 채팅 UI |
...
PDF에서 테이블을 LLM이 이해하기 좋은 텍스트로 변환합니다.

**Step 1 — 테이블 영역 감지** (PyMuPDF)

PDF의 선(line), 배경색, 텍스트 정렬 패턴을 분석해 테이블 영역을 일반 텍스트와 분리합니다.
감지된 테이블마다 bbox 좌표를 기록해 두어 이후 교차 페이지 병합 판단에도 활용합니다.

**Step 2 — 제목 감지** (휴리스틱)

테이블 상단에서 가장 가까운 텍스트 블록(120자 이하)을 캡션으로 추출합니다.
...
| `payload.metadata.doc` | 원본 PDF 파일명 |
...
app/streamlit_app.py
    └─► app/rag_agent.py ──────────────────────────────────────────┐
                                                                    ▼
tools/pdf_to_pptx.py ──► table_rag.table.*                    table_rag.qa
                                                               table_rag.retrieval
table_rag.indexing ──────────────────────────────────────────► table_rag.document
table_rag.retrieval ─────────────────────────────────────────► table_rag.models
table_rag.document ──────────────────────────────────────────► table_rag.table.*
table_rag.table.* ───────────────────────────────────────────► pymupdf / python-pptx
```
...
# BM25 하이브리드 검색 추가 (Python 3.10–3.12 권장)
...
4. 답변 하단에 경로 표시: `📄 RAG 검색 (벡터 DB)` / `🌐 웹 검색 (문서 부족 시 폴백)`

---

## 9. 코드 재사용 가이드

이 레포지토리의 코드는 두 레이어로 분리되어 있습니다.

| 레이어 | 위치 | 재사용 가능 여부 |
|--------|------|----------------|
| **테이블 유틸리티** | `table_rag/table/` | ✅ Qdrant·LLM 의존성 없음, pymupdf만 필요 |
| **문서 로더** | `table_rag/document/` | ✅ 청크 추출만 필요한 경우 |
| **전체 RAG 파이프라인** | `table_rag/{indexing,retrieval,qa}.py` | ✅ Qdrant + OpenAI 필요 |
| **LangGraph 에이전트** | `app/rag_agent.py` | ⚠ 프로젝트 전용 (노드 로직 교체 필요) |
| **Streamlit UI** | `app/streamlit_app.py` | ⚠ 프로젝트 전용 |

---

### 시나리오 1 — PDF 테이블만 추출하기 (RAG 없이)

가장 가볍게 재사용할 수 있는 패턴입니다.  
`pymupdf`만 필요하며 Qdrant·OpenAI 키가 없어도 동작합니다.
...
doc    = fitz.open("재무보고서.pdf")
raw    = extract_raw_tables_from_doc(doc)       # 페이지별 테이블 추출
merged = merge_cross_page_raw_tables(raw)       # 교차 페이지 병합

for t in merged:
    print(f"=== {t['title']} (p.{t['page']}~{t['end_page']}) ===")

    # ① LLM / 임베딩용 텍스트 직렬화
    print(table_to_text(t["rows"], table_title=t["title"]))

    # ② PPTX·DataFrame 등 다른 포맷용 (header, body) 분리
...
### 시나리오 2 — pandas DataFrame으로 변환하기
...
merged = merge_cross_page_raw_tables(extract_raw_tables_from_doc(doc))
...
for t in merged:
    ...
    if header and body:
        ...
        df["_table_title"] = t["title"]
        df["_page"]        = t["page"]
        dataframes.append(df)

# 모든 테이블을 하나로 합치거나 개별 접근
...
### 시나리오 3 — PPTX·PPT 파일 테이블 추출

```python
from pathlib import Path
from table_rag.document import load_pptx_chunks

chunks = load_pptx_chunks(Path("발표자료.pptx"))

for chunk in chunks:
    if chunk.source_type == "table":
        print(f"[슬라이드 {chunk.page}] {chunk.text[:200]}")
```

---

### 시나리오 4 — 문서 청킹만 사용하기 (인덱싱 없이)

```python
from pathlib import Path
from table_rag.document import load_document_chunks

# PDF / PPTX 자동 감지
chunks = load_document_chunks(
    Path("연간보고서.pdf"),
    doc_slug="annual_2026",
    extract_table_chunks=True,
...
### 시나리오 5 — 자체 벡터 DB로 인덱싱하기

`table_rag.table` + `table_rag.document` 로 청크를 뽑은 뒤,  
Qdrant 대신 자신의 벡터 DB(예: Chroma, Weaviate, pgvector)에 저장할 수 있습니다.
...
# ─ 여기서부터 자신의 DB 코드로 교체 ─
# your_vector_db.upsert(records, embed_fn=your_embed_fn)
```

---

### 시나리오 6 — 전체 RAG 파이프라인 재사용

Qdrant + OpenAI 환경이 준비된 경우 전체 파이프라인을 그대로 가져다 씁니다.
...
# 1. 인덱싱 (최초 1회)
ingest_pdfs_to_qdrant(
    pdf_paths=["보고서.pdf", "계약서.pdf"],
    collection="my_collection",
    enable_hybrid=True,          # BM25 + 벡터 하이브리드 (fastembed 필요)
    extract_table_chunks=True,
    merge_cross_page_tables=True,
)

# 2. 검색
docs, warn = search_and_rerank(
    query="영업이익은 얼마인가요?",
    collection="my_collection",
    qdrant_top_k=20,
    rerank_top_k=5,
...
answer = answer_with_openai(query="영업이익은 얼마인가요?", docs=docs, llm=llm)
print(answer)
```

---

### 시나리오 7 — 단일 문서 추가 (컬렉션 유지)

전체 재인덱싱 없이 문서 하나만 교체할 때 사용합니다.
...
#### `table_rag.table` (RAG 무관, 독립 사용 가능)

| 함수 | 설명 |
|------|------|
| `extract_raw_tables_from_doc(doc)` | PyMuPDF doc → raw table dict 리스트 |
| `merge_cross_page_raw_tables(raw)` | 교차 페이지 테이블 병합 |
| `find_table_title(page, table)` | 테이블 위 제목 텍스트 탐색 |
| `table_to_text(rows, title, ...)` | LLM/임베딩용 구조화 텍스트 직렬화 |
| `normalize_table(rows)` | → `(header: List[str], body: List[List[str]])` |
| `normalize_cell(value)` | 셀 값 공백 정규화 |
| `fill_rowspan_cells(body)` | PyMuPDF rowspan 빈 셀 소급 채우기 |
| `combine_header_rows(row0, row1)` | 2단 헤더 병합 |

#### `table_rag.document`

| 함수 | 설명 |
|------|------|
| `load_document_chunks(path, doc_slug, ...)` | PDF/PPTX → `List[Chunk]` (자동 감지) |
| `load_pdf_chunks(path, ...)` | PDF 전용 |
| `load_pptx_chunks(path, ...)` | PPTX/PPT 전용 |
| `chunk_text(text, chunk_size, overlap)` | 슬라이딩 윈도우 텍스트 분할 |
| `sanitize_doc_stem(stem)` | 파일명 → 안전한 slug |
| `assign_doc_slugs(paths)` | 다중 파일에 충돌 없는 slug 할당 |

#### `table_rag` (전체 파이프라인)

| 함수 | 설명 |
|------|------|
| `ingest_pdfs_to_qdrant(pdf_paths, collection, ...)` | 전체 재인덱싱 |
| `ingest_pdf_to_qdrant(pdf_path, collection, ...)` | 단일 문서 전체 재인덱싱 |
| `upsert_document_to_qdrant(doc_path, collection, ...)` | 단일 문서 추가/교체 |
| `search_and_rerank(query, collection, ...)` | 하이브리드 검색 + 리랭킹 |
| `answer_with_openai(query, docs, llm, ...)` | LLM 답변 생성 |
| `build_context_from_docs(docs, ...)` | 검색 결과 → 컨텍스트 문자열 |

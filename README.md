...
이 프로젝트는 테이블을 별도 청크로 구조화하고, LangGraph 기반 자동 라우팅으로 질문 유형에 맞는 최적 경로(벡터 DB 검색 또는 웹 검색)를 선택합니다.
...
| 9 | `0651e7d` | LangGraph 에이전트 도입 | Router → 문서 RAG / Web RAG 자동 분기, Tavily 웹검색 |
...
| `web_fallback` | 🌐 웹 검색 (폴백) | Qdrant 검색했으나 관련 문서 없음 → Tavily |
...
기존 시스템은 모든 질문에 벡터 DB 검색을 수행했습니다. 질문 유형에 따라 최적 경로가 다릅니다.

| 질문 유형 | 예시 | 최적 경로 |
|-----------|------|-----------|
| 문서 관련 | "포스코홀딩스 주식 소각 수량은?" | 벡터 DB 검색 → LLM |
| 일반/최신 정보 | "2026년 AI 트렌드는?" | 웹검색(Tavily) → LLM |
| 문서 검색 실패 | 검색 문서가 질문과 무관할 때 | 웹검색(Tavily) → LLM (폴백) |
...
flowchart TD
    ...
    RouterNode -->|"RAG 주제"| RetrieveNode["retrieve_node\n벡터 DB 검색\nQdrant + 리랭커"]
    RouterNode -->|"일반 주제"| WebDirectNode["web_rag_node\nTavily 웹검색\n+ LLM 답변 생성"]
    ...
    WebDirectNode --> B([source: web_direct])
    WebFallbackNode --> C([source: web_fallback])
```
...
```
START → router_node
  ├─ route == "rag"     → retrieve_node → grade_node
...
#### `router_node` — RAG vs 일반 분류

LLM이 질문을 보고 벡터 DB 검색(`"rag"`)이 필요한지, 웹검색(`"general"`)이 더 적합한지 판단합니다.

| 질문 | 분류 | 이유 |
|------|------|------|
| "포스코홀딩스 주식 소각 수량은?" | `rag` | 특정 공시 문서 내용 |
| "주주총회 의안 내용은?" | `rag` | 업로드된 PDF 내 정보 |
| "2026년 글로벌 철강 시장 전망은?" | `general` | 최신 시장 정보 |
| "포스코 현재 주가는?" | `general` | 실시간 정보, 웹검색 필요 |
...
#### `web_rag_node` — Tavily 웹검색 + LLM

Tavily API로 최대 5개 결과 수집 → URL + 내용 조합 → LLM 답변 + 참고 URL 표시. RAG 경로에서 문서 부족으로 진입하면 폴백, 일반 주제로 바로 진입하면 직접 호출로 구분됩니다.

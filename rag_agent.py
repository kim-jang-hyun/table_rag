...
아키텍처:
  사용자 질문
      └─► retrieve_node (벡터 DB 검색)
              └─► grade_node (문서 품질 평가)
                      ├─► [충분] generate_node → 최종 응답  (rag_generate)
                      └─► [부족] web_rag_node  → 최종 응답  (web_fallback)

모든 질문은 먼저 벡터 DB에서 검색하고, 문서 품질이 부족할 때만 Tavily 웹검색으로 폴백합니다.
"""
...
if not _perf_logger.handlers:
    ...
    # 로컬 환경(STREAMLIT_SERVER_HEADLESS 미설정)에서만 파일 로그 활성화
